...
const state = {
  ...
  songProgress: 37,
  notifications: [
    { id: crypto.randomUUID(), app: 'kakao', sender: '유안', message: '아직 안 잤어?', time: '지금', icon: '' },
    { id: crypto.randomUUID(), app: 'instagram', sender: 'seoluS2', message: '회원님의 스토리를 좋아합니다.', time: '12분 전', icon: '' },
  ],
};
...
const apps = {
  ...
  discord: { name: 'Discord', icon: svgIcon('#5865F2', `<path d="M7.3 7.2c2-1.48 7.35-1.48 9.35 0 .72 1.16 1.22 2.47 1.46 3.84-.96 1.2-2.3 2.19-3.9 2.81l-.48-.65c.58-.22 1.12-.5 1.6-.83-1.77.83-4.65.83-6.42 0 .48.33 1.02.61 1.6.83l-.48.65c-1.6-.62-2.94-1.6-3.9-2.81.24-1.37.74-2.68 1.46-3.84ZM9.6 10.7c.65 0 1.18-.62 1.18-1.38S10.25 7.94 9.6 7.94s-1.18.62-1.18 1.38.53 1.38 1.18 1.38Zm4.8 0c.65 0 1.18-.62 1.18-1.38s-.53-1.38-1.18-1.38-1.18.62-1.18 1.38.53 1.38 1.18 1.38Z" fill="#fff"/>`) },
...
function renderNotifications() {
  ...
  if (!state.notifications.length) {
    stack.innerHTML = '<div class="empty-notification">알림 없음</div>';
    return;
  }
  const groups = [];
...
function renderPhone() {
  ...
  $('#notificationPositionValue').textContent = state.notificationY < 300 ? '위쪽' : state.notificationY > 480 ? '아래쪽' : '중간';
  renderNotifications();
...
function bindControls() {
  ...
  $('#musicMode').addEventListener('change', event => { state.musicMode = event.target.value; renderPhone(); });
  ...
  layer.addEventListener('pointermove', event => {
    if (!drag) return;
    const next = Math.min(620, Math.max(190, drag.startTop + event.clientY - drag.startY));
